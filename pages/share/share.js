...
//生命周期函数--监听页面加载  
  onLoad(options) {
      ...
      this.setData({
          ...
          scale:decodeURIComponent(options.scale),   
          ...
          wxName:decodeURIComponent(options.wxName)
      })
      let b = 750/a.globalData.screen;
      this.setData({
          styleText:`width:${200*this.data.scale *1.171875}rpx;height:${200*this.data.scale*1.171875}rpx;font-size:${this.data.fontSize*this.data.scale*1.171875}rpx;line-height:${this.data.leading*this.data.scale*1.171875}rpx;left:${this.data.x*b*1.171875}rpx;top:${this.data.y*b*1.171875}rpx;border: ${1*this.data.scale*1.171875}rpx solid rgba(5, 5, 5, 0);`
      })
  },
// 生命周期函数--监听页面初次渲染完成
  onReady() {},
...
// 用户点击右上角分享
  onShareAppMessage: function( options ){
    ...
    let shareObj = {
      ...
      path: `/share/share?url=${encodeURIComponent(this.data.url)}&oppositeurl=${encodeURIComponent(this.data.oppositeurl)}&scale=${encodeURIComponent(this.data.scale)}&fontSize=${encodeURIComponent(this.data.fontSize*2/3)}&text=${encodeURIComponent(this.data.text)}&x=${encodeURIComponent(this.data.x)}&y=${encodeURIComponent(this.data.y)}&leading=${encodeURIComponent(this.data.leading)}`,   // 默认是当前页面，必须是以‘/'开头的完整路径
